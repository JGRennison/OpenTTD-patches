...
{
	if (this->status < ServerStatus::Encrypted || this->status >= ServerStatus::AuthCompany) return NETWORK_RECV_STATUS_MALFORMED_PACKET;
...
{
	if (this->status == ServerStatus::Closing) return NETWORK_RECV_STATUS_OKAY;
