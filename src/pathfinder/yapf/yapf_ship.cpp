...
public:
	void SetDestination(const Ship *v)
	{
		if (v->current_order.IsType(OT_GOTO_STATION)) {
			...
			this->dest_trackdirs = GetTileTrackdirBits(this->dest_tile, TransportType::Water, 0);
		}
	}
...
public:
	...
	static bool CheckShipReverse(const Ship *v, Trackdir *trackdir)
	{
		...
		if (trackdir == nullptr) {
			...
			const TrackdirBits reverse_dirs = DiagdirReachesTrackdirs(entry) & GetTileTrackdirBits(v->tile, TransportType::Water, 0, entry);
