	...
	int DrawInfo(const Rect &r)
	{
		...
		for (const auto &p : i->Produced()) {
			...
			if (this->editable == Editability::Rate) {
				DrawArrowButtons(ir.Indent(label_indent, rtl).WithWidth(SETTING_BUTTON_WIDTH, rtl).left, ir.top + button_y_offset, COLOUR_YELLOW, (this->clicked_line == (&p - i->produced.get())) ? this->clicked_button : 0,
						p.rate > 0, p.rate < 255);
			}
			ir.top += line_height;
		}
