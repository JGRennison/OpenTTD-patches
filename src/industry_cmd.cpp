...
Industry::~Industry()
{
	...
	if (_game_mode == GameMode::Normal) RegisterGameEvents(GEF_INDUSTRY_DELETE);
}
...
static void DoCreateNewIndustry(Industry *i, TileIndex tile, IndustryType type, const IndustryTileLayout &layout, size_t layout_index, Town *t, Owner founder, uint16_t initial_random_bits)
{
	...
	if (_game_mode == GameMode::Normal) RegisterGameEvents(GEF_INDUSTRY_CREATE);
}
...
int WhoCanServiceIndustry(Industry *ind)
{
	...
	for (const Vehicle *v : Vehicle::Iterate()) {
		...
		if (v->type == VehicleType::Train && v->IsFrontEngine() && !HasBit(v->subtype, GVSF_VIRTUAL)) {
		...
		for (const Order *o : v->Orders()) {
			if (o->IsType(OT_GOTO_STATION) && o->GetUnloadType() != OrderUnloadType::Transfer) {
				...
				if ((o->GetUnloadType() == OrderUnloadType::NoUnload) && !c_accepts) break;
