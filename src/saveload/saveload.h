...
enum class SaveLoadFlag : uint8_t {
	Global,
};
...
struct SaveLoad {
	...
	union {
		 ...
		 * that is called to save it. address: SaveLoadFlag::Global set, offset: SaveLoadFlag::Global clear */
		void *address; ///< address of variable (SaveLoadFlag::Global set)
		size_t offset; ///< offset of variable in the struct (SaveLoadFlag::Global clear)
	};
...
template <class T, template <class, class...> class U>
inline constexpr bool SlCheckValueContainerType(VarMemType type)
{
	if constexpr (SlIsContainerType<T, U>()) {
 ...
 */
template <typename T>
inline constexpr bool SlCheckVarSize(SaveLoadType cmd, VarMemType type, size_t length)
{
...
template <SaveLoadType cmd, VarMemType type, size_t length, typename T>
...
#define SLE_GENERAL_NAME(cmd, name, base, variable, type, length, from, to, extra) \
	SaveLoad {name, cmd, SaveLoadFlags{}, type, length, from, to, { .offset = SlVarWrapper<cmd, VarType{type}.mem, length, decltype(base::variable)>(cpp_offsetof(base, variable)) }, nullptr}
...
#define SLE_VAR2(base, name, variable, type) SLE_CONDVARNAME(base, variable, name, type, SaveLoadVersion::MinVersion, SaveLoadVersion::MaxVersion)
 ...
 * @param length   Number of elements in the string (only used for fixed size buffers).
 */
#define SLE_STR(base, variable, type, length) SLE_CONDSTR(base, variable, type, length, SaveLoadVersion::MinVersion, SaveLoadVersion::MaxVersion)
 ...
 * @param type     Storage of the data in memory and in the savegame.
 */
#define SLE_REFRING(base, variable, type) SLE_CONDREFRING(base, variable, type, SaveLoadVersion::MinVersion, SaveLoadVersion::MaxVersion)
...
#define SLEG_GENERAL(name, cmd, variable, type, length, from, to, extra) \
	SaveLoad {name, cmd, SaveLoadFlag::Global, type, length, from, to, { .address = SlVarWrapper<cmd, VarType{type}.mem, length, decltype(variable)>(static_cast<void *>(std::addressof(variable))) }, nullptr}
 ...
 */
#define SLEG_CONDSTRUCT(name, handler, from, to) SaveLoad {name, SaveLoadType::Struct, SaveLoadFlags{}, {}, 0, from, to, { .address = nullptr }, std::make_shared<handler>()}
 ...
 * @param to       Last savegame version that has the list.
 */
#define SLEG_CONDSTRUCTLIST(name, handler, from, to) SaveLoad {name, SaveLoadType::StructList, SaveLoadFlags{}, {}, 0, from, to, { .address = nullptr }, std::make_shared<handler>()}
 ...
 * @param type     Storage of the data in memory and in the savegame.
 */
#define SLEG_STR(name, variable, type) SLEG_CONDSTR(name, variable, type, sizeof(variable), SaveLoadVersion::MinVersion, SaveLoadVersion::MaxVersion)
 ...
 * @param type     Storage of the data in memory and in the savegame.
 */
#define SLEG_REFRING(name, variable, type) SLEG_CONDREFRING(name, variable, type, SaveLoadVersion::MinVersion, SaveLoadVersion::MaxVersion)
...
{
	return SaveLoadVersion{_sl_version} < major || (minor > 0 && SaveLoadVersion{_sl_version} == major && _sl_minor_version < minor);
}
...
{
	return SaveLoadVersion{_sl_version} <= major;
}
...
inline void *GetVariableAddress(const void *object, const SaveLoad &sld)
{
	...
	if (sld.flags.Test(SaveLoadFlag::Global)) return sld.address;
