...
void PickerWindow::SetDisabledRandomItemButton()
{
	...
	this->SetWidgetDisabledState(WID_PW_TYPE_RANDOM, !HasBit(this->callbacks.mode, PFM_SAVED) || (this->callbacks.saved.contains(this->callbacks.sel_collection) ? !this->callbacks.IsCollectionValidForRandom(this->callbacks.saved[this->callbacks.sel_collection], this) : true));
...
void PickerWindow::OnClick(Point pt, WidgetID widget, int)
{
	switch (widget) {
		...
		case WID_PW_CONFIGURE_BADGES:
			...
			ShowDropDownList(this, BuildBadgeClassConfigurationList(this->badge_classes, 1, {}), -1, widget, 0, DDMF_PERSIST);
			break;
		...
		NWidget(NWID_SELECTION, INVALID_COLOUR, WID_PW_CLASS_SEL),
			NWidget(NWID_VERTICAL),
				NWidget(WWT_PANEL, COLOUR_DARK_GREEN),
					NWidget(WWT_EDITBOX, COLOUR_DARK_GREEN, WID_PW_CLASS_FILTER), SetMinimalSize(240, 0), SetPadding(2), SetFill(1, 0), SetStringTip(STR_LIST_FILTER_OSKTITLE, STR_LIST_FILTER_TOOLTIP),
				EndContainer(),
				/* Collection view */
				NWidget(NWID_VERTICAL),
