...
void SchDispatchBulkAddCmdData::Serialise(BufferSerialisationRef buffer) const
{
	buffer.Send_generic_seq(this->veh, this->schedule_index);
	buffer.Send_generic(this->slots.size());
	for (const auto &it : this->slots) {
		buffer.Send_generic_seq(it.first, it.second);
	}
}

bool SchDispatchBulkAddCmdData::Deserialise(DeserialisationBuffer &buffer, StringValidationSettings default_string_validation)
{
	buffer.Recv_generic_seq(default_string_validation, this->veh, this->schedule_index);
	size_t size = 0;
	buffer.Recv_generic(size, default_string_validation);
	if (size > 512) return false;

	this->slots.resize(size);
	for (auto &it : this->slots) {
		buffer.Recv_generic_seq(default_string_validation, it.first, it.second);
	}
	return true;
}

void SchDispatchBulkAddCmdData::FormatDebugSummary(format_target &output) const
{
	output.format("{}, {}, {} slots", this->veh, this->schedule_index, this->slots.size());
}

/**
 * Bulk add scheduled dispatch time offsets
 * @param flags Operation to perform.
 * @param data Command data.
 * @return the cost of this operation or an error
 */
CommandCost CmdSchDispatchBulkAdd(DoCommandFlags flags, const SchDispatchBulkAddCmdData &data)
{
	Vehicle *v = Vehicle::GetIfValid(data.veh);
	if (v == nullptr || !v->IsPrimaryVehicle()) return CMD_ERROR;
	...
	if (data.schedule_index >= v->orders->GetScheduledDispatchScheduleCount()) return CMD_ERROR;
	...
	if (flags.Test(DoCommandFlag::Execute)) {
		DispatchSchedule &ds = v->orders->GetDispatchScheduleByIndex(data.schedule_index);
		std::vector<DispatchSlot> &dslist = ds.GetScheduledDispatchMutable();
		uint32_t after_end = dslist.empty() ? 0 : dslist.back().offset + 1;
		for (const auto &it : data.slots) {
			if (it.first >= after_end) {
				/* Append to end, fast path */
				dslist.push_back({ it.first, it.second });
				after_end = it.first + 1;
			} else {
				auto insert_position = std::lower_bound(dslist.begin(), dslist.end(), DispatchSlot{ it.first, 0 });
				if (insert_position != dslist.end() && insert_position->offset == it.first) {
					insert_position->flags = it.second;
				} else {
					dslist.insert(insert_position, { it.first, it.second });
				}
			}
		}
		ds.UpdateScheduledDispatch(nullptr);
