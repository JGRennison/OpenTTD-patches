...
private:
	static void LogErrorBufferTooShort(size_type len, size_type size);
...
public:
	...
	[[nodiscard]] std::string_view Read(size_type len)
	{
		...
		if (len != npos && len != result.size()) {
			LogErrorBufferTooShort(len, result.size());
		}
...
private:
	static void LogErrorIntegerOutOfRange(std::string_view str);
	...
	[[nodiscard]] static std::pair<size_type, T> ParseIntegerBase(std::string_view src, int base, bool log_errors)
	{
		...
		if (result.ec == std::errc::result_out_of_range) {
			if (log_errors) LogErrorIntegerOutOfRange2(src.substr(0, len), src.substr(len, 4));
			return {};
		}
