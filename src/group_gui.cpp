...
private:
	...
	void DrawGroupInfo(Rect r, GroupID g_id, uint16_t level_mask = 0, uint8_t indent = 0, bool protection = false, bool has_children = false) const
	{
		...
		if (has_children) {
			DrawSpriteIgnorePadding(Group::Get(g_id)->IsFolded(GroupFoldBits::GroupView) ? SPR_CIRCLE_FOLDED : SPR_CIRCLE_UNFOLDED, PAL_NONE, r.WithWidth(this->column_size[VGC_FOLD].width, rtl), SA_CENTER);
		}
			...
			case WID_GL_RENAME_GROUP: // Rename the selected group
				if (_ctrl_pressed) {
					...
					if (!this->vehgroups.empty()) {
						...
						if (!name.empty()) {
							Command<CMD_ALTER_GROUP>::Post(STR_ERROR_GROUP_CAN_T_RENAME, AlterGroupMode::Rename, this->vli.ToGroupID(), GroupID::Invalid(), name);
							return;
						}
					}
				}
