...
public:
		...
		switch (widget) {
			...
			case WID_SL_LOAD_BUTTON: {
				...
				if (this->abstract_filetype == FT_HEIGHTMAP) {
					...
					if (file.has_value()) {
						...
						importJsonOrderList(veh,buffer.str());
					}
			...
			case WID_SL_SAVE_GAME: // Save game
				...
				if (this->abstract_filetype == FT_ORDERLIST) {
					...
					std::string fileName = FiosMakeOrderListName(this->filename_editbox.text.GetText());
					std::ofstream output;
	...
	void OnTimeout() override
	{
		...
		if (this->IsWidgetLowered(WID_SL_DELETE_SELECTION)) { // Delete button clicked
			if (!FiosDelete(this->filename_editbox.text.GetText(),this->abstract_filetype)) {
