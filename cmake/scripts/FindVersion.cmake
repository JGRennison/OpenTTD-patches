...
if(GIT_FOUND AND EXISTS "${CMAKE_SOURCE_DIR}/.git")
    ...
    if(EXISTS "${CMAKE_SOURCE_DIR}/.ottdrev-vc-dbpp")
        file(READ "${CMAKE_SOURCE_DIR}/.ottdrev-vc-dbpp" OTTDREVVC)
        string(REPLACE "\n" ";" OTTDREVVC "${OTTDREVVC}")
        list(GET OTTDREVVC 0 OTTDREV)
        string(REPLACE "\t" ";" OTTDREV "${OTTDREV}")
        list(GET OTTDREV 0 REV_RELEASE)
    else()
        set(REV_RELEASE "dbpp-0.0")
    endif()

    # Restore LC_ALL
    set(ENV{LC_ALL} "${SAVED_LC_ALL}")
elseif(EXISTS "${CMAKE_SOURCE_DIR}/.ottdrev-vc-dbpp")
    file(READ "${CMAKE_SOURCE_DIR}/.ottdrev-vc-dbpp" OTTDREVVC)
    string(REPLACE "\n" ";" OTTDREVVC "${OTTDREVVC}")
    list(GET OTTDREVVC 0 OTTDREV)
...
if(EXISTS "${CMAKE_SOURCE_DIR}/.ottdrev-vc")
    ...
    list(GET OTTDREVVC 1 SRCHASH)
    string(REPLACE "\t" ";" OTTDREV "${OTTDREV}")
    list(GET OTTDREV 0 JGRPP_REV_VERSION)
    list(GET OTTDREV 0 JGRPP_REV_RELEASE)
else()
    message(WARNING "No JGRPP version detected; this build will NOT be network compatible")
    set(JGRPP_REV_VERSION "norev0000")
    set(JGRPP_REV_RELEASE "jgrpp-0.0")
endif()


string(REGEX MATCH "^dbpp-[0-9]+(\.[0-9]+)?(\.[0-9]+)?" REV_RELEASE "${REV_RELEASE}")
string(REPLACE "dbpp-" "" REV_RELEASE "${REV_RELEASE}")

string(REGEX MATCH "^jgrpp-[0-9]+(\.[0-9]+)?(\.[0-9]+)?" JGRPP_REV_RELEASE "${JGRPP_REV_RELEASE}")
string(REPLACE "jgrpp-" "" JGRPP_REV_RELEASE "${JGRPP_REV_RELEASE}")

message(STATUS "Version string: ${REV_VERSION}, Release: ${REV_RELEASE}, HGRPP Release: ${JGRPP_REV_RELEASE}")
